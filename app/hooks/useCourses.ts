...
const COURSE_COLORS = [
  'bg-blue-500/20 border-blue-500/30', 'bg-sky-500/20 border-sky-500/30',
  'bg-purple-500/20 border-purple-500/30', 'bg-orange-500/20 border-orange-500/30',
  'bg-amber-500/20 border-amber-500/30', 'bg-green-500/20 border-green-500/30',
  'bg-emerald-500/20 border-emerald-500/30', 'bg-slate-500/20 border-slate-500/30',
  'bg-rose-500/20 border-rose-500/30', 'bg-yellow-500/20 border-yellow-500/30',
  'bg-red-500/20 border-red-500/30', 'bg-indigo-500/20 border-indigo-500/30',
  'bg-cyan-500/20 border-cyan-500/30', 'bg-lime-500/20 border-lime-500/30',
];
...
export function useCourses(options: UseCoursesOptions = {}): UseCoursesReturn {
  ...
  useEffect(() => {
    ...
    const loadCourses = async () => {
      try {
        const res = await fetch(`/api/supabase?type=courses&user_id=${userId}`);
        if (res.ok) {
          const data = await res.json();
          if (Array.isArray(data) && data.length > 0) {
            const converted: Course[] = data.map((c: Record<string, unknown>) => ({
              id: c.id as string,
              day: DAY_REVERSE_MAP[c.day_of_week as number] || '周一',
              subject: c.name as string,
              time: `${(c.start_time as string)?.substring(0, 5) || ''}-${(c.end_time as string)?.substring(0, 5) || ''}`,
              type: (c.location === '课外' ? '课外' : '校内') as '校内' | '课外',
              color: (c.color as string) || COURSE_COLORS[0],
            }));
  ...
  const addCourse = useCallback(() => {
    ...
    const course: Course = { ...newCourse, color: COURSE_COLORS[colorIndex] };
    ...
    if (USE_SUPABASE && userId) {
      ...
      sync('courses', {
        ...
        data: {
          ...
          location: newCourse.type === '课外' ? '课外' : '校内',
        },
      }).catch(() => {});
    }
    setNewCourse({ day: selectedDay, subject: '', time: '', type: '校内' });
    setIsAddingCourse(false);
  }, [newCourse, courses.length, selectedDay, userId, sync]);
  ...
  return {
    ...
    setScheduleView, setSelectedDay, setIsAddingCourse, setNewCourse,
