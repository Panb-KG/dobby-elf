...
import { QUICK_ACTIONS, getSortedActions, type LeftTab } from './v2-constants';

const ConversationList = dynamic(() => import('@/components/v2/ConversationList'), {
  loading: () => null,
  ssr: false,
});
...
export const LeftSidebar = memo(function LeftSidebar({
  ...
  currentConvId, onSwitchConversation, onNewConversation,
  sidebarMode, onToggleSidebarMode,
}: LeftSidebarProps) {
  ...
  return (
    <div className={`flex-shrink-0 transition-all duration-300 border-r border-orange-900/30 ${
      ...
      {!isLeftCollapsed ? (
        <>
          {/* 模式切换按钮 */}
          <div className="flex border-b border-orange-900/20">
            <button
              onClick={() => setShowMenu(false)}
              className={`flex-1 px-3 py-2 text-xs font-medium transition-colors ${
                !showMenu ? 'text-orange-400 border-b-2 border-orange-400' : 'text-gray-400 hover:text-gray-300'
              }`}
            >
        ...
        <div className="flex-1 flex flex-col items-center py-4 gap-4">
          ...
          <button
            onClick={() => { onToggleCollapse(); setShowMenu(true); }}
            className="p-2 rounded-lg hover:bg-white/5 text-gray-400"
            title="菜单"
          >
